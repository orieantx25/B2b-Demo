...
export default function AdminTargetsPage() {
  ...
  return (
    <div className="animate-in pb-16">
      <PageHeader
        title="User targets"
        subtitle="Set schools, consultants, meetings, and coachings targets per B2B user. Achievement appears only in Reports — not on the B2B portal."
      />

      <div className="mb-4 max-w-md">
        <Label htmlFor="target-search">Search users</Label>
        <Input
          id="target-search"
          value={q}
      ...
      {list.length === 0 ? (
        <EmptyState title="No B2B users" description="Add B2B members under Users & Roles first." />
      ) : (
        <>
      ...
      >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {(
            ...
            <div key={key}>
              <Label htmlFor={`t-${key}`}>{label} target</Label>
              <Input
                id={`t-${key}`}
                type="number"
                min={0}
